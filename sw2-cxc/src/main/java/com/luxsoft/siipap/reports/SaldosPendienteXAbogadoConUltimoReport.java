 ...
 *
 */
public class SaldosPendienteXAbogadoConUltimoReport extends SWXAction{
	...
	protected void execute() {
		...
		if(!form.hasBeenCanceled()){
			ReportUtils.viewReport(ReportUtils.toReportesPath("cxc/SaldosPendientesPorAbogadoConUltPag.jasper"), form.getParametros());
		}
		form.dispose();
	}
...
private ParamLabelValue getParam(JComboBox box){
		...
		private void initComponents(){
			...
			Object abogados[]={//"MARTHA SILVA CASARIN","JOSE ANTONIO JACUINDE GUTIERREZ","CENTRAL DE COBRANZA","FRANCISCO FRIAS ( 2000 PLUS)"
					new ParamLabelValue("ALEJANDRO LEZAMA BRACHO","ALEJANDRO LEZAMA BRACHO"),
	...
	public static void run(){
		SaldosPendienteXAbogadoConUltimoReport action=new SaldosPendienteXAbogadoConUltimoReport();
		action.execute();
	}
