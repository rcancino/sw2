...
public class ClientesBrowser2 extends SXAbstractDialog{
	...
	public List<Cliente> findData(){
		if(nombreDigitado()){
			return ServiceLocator2.getClienteManager().buscarClientePorNombre(nombreField.getText());			
		}else if(claveDigitada()){
			return ServiceLocator2.getClienteManager().buscarClientePorClave(claveField.getText());
		}else
	...
	public void doSelect(){
		...
		if(index!=-1){
			...
			Cliente target=ServiceLocator2.getClienteManager().get(c.getId());
