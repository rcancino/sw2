 ...
 *
 */
public class ImpresionDeNotaDescuento implements HibernateCallback{
	...
	public ImpresionDeNotaDescuento(String id) {
		this.id = id;
	}
	...
	public  void imprimir(final NotaDeCredito nota) throws Exception {
		...
		for (Aplicacion det : nota.getAplicaciones()) {
			...
			String importe;
			if(det.getCargo()!=null){
				importe = nf.format(det.getCargo().getImporte().add(devoluciones.amount()).abs().doubleValue());//.toString(); //
			...
			String desc = nf.format(Math.abs(nota.getDescuento()*100));
			//String desc = StringUtils.leftPad(nota.getDescuento(), 5);
