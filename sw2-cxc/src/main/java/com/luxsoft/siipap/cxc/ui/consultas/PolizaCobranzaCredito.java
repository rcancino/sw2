...
import com.luxsoft.siipap.cxc.model.PolizaCobranzaCreditoModel;
import com.luxsoft.siipap.cxc.util.PolizaUtils;
...
public class PolizaCobranzaCredito extends AbstractMasterDatailFilteredBrowserPanel<Poliza, AsientoContable> {
	
	private PolizaCobranzaCreditoModel model;
	
	public PolizaCobranzaCredito() {
		super(Poliza.class);
		model=new PolizaCobranzaCreditoModel();
	}
	
	protected void init(){
		
		addProperty(new String[]{"fecha","tipo","concepto","debe","haber","cuadre","year","mes"});
		addLabels(  new String[]{"Fecha","Tipo","Concepto","Debe","Haber","Cuadre","year","mes"});
	...
	private String prefijoDeArchivo="C";
	
	public void generarPoliza(){
		if(!getSelected().isEmpty()){
			for(Object o:getSelected()){
				Poliza poliza=(Poliza)o;
				PolizaUtils.exportarPoliza(poliza, prefijoDeArchivo);
			}
		}
	}
	...
	private class EliminarRegistroPredicate implements Predicate{
