...
public class ImpresionDeNotaDevolucion implements HibernateCallback{
	...
	public ImpresionDeNotaDevolucion(String id,int folio,Date fecha) {
		this(id);
	}
	...
	public  void imprimir(final NotaDeCreditoDevolucion nota) throws Exception {
		...
		insertarDetalleParaDevolucion_BAK(nota, p);
	...
	public static void insertarDetalleParaDevolucion_BAK(final NotaDeCreditoDevolucion nota,final PrintWriter p){
		...
		double subtotal=0;
		List<DevolucionDeVenta> partidas=ServiceLocator2.getHibernateTemplate().find("from DevolucionDeVenta det where det.nota.id=?",nota.getId());
