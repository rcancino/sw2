...
public class PedidoDetDolaresFormModel extends DefaultFormModel implements PropertyChangeListener{
	...
	public PedidoDetDolaresFormModel(PedidoDet bean) {
		super(bean);
	}
	...
	protected void init(){
		...
		getModel("cantidad").addValueChangeListener(this);
		getModel("producto").addValueChangeListener(this);
		getModel("precio").addValueChangeListener(this);
	}
	...
	protected void actualizarExistencias(){
		...
		SwingWorker<List<Existencia>, String> worker=new SwingWorker<List<Existencia>, String>(){			
			protected List<Existencia> doInBackground() throws Exception {
				logger.info("Actualizando existencias para: "+getPedidoDet().getProducto().getClave());
			...
			protected void done() {
				try {
					...
					logger.error("Error actualizando existencias del producto: "+getPedidoDet().getProducto());
				}
				existenciaTotal=0;
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		}else if("precio".equals(evt.getPropertyName())){
			getPedidoDet().actualizar();
