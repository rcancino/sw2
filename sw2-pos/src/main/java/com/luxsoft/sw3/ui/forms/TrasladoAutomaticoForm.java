 ...
 *
 */
public class TrasladoAutomaticoForm extends AbstractForm {

	public TrasladoAutomaticoForm(final TrasladoAutomaticoController model) {
		super(model);
	...
	public TrasladoAutomaticoController getController(){
		return (TrasladoAutomaticoController)getModel();
	}
	...
	private EventSelectionModel<TrasladoDet> selectionModel;
	
	protected JComponent buildGridPanel(){
		String[] propertyNames={"producto.clave","producto.descripcion","cantidad","comentario","cortes","instruccionesDecorte"};
		String[] columnLabels={"Producto","Descripcin","Cantidad","Comentario","Cortes","Instruccion"};
		boolean[] edits={false,false,false,false,false,false};
		final TableFormat tf=GlazedLists.tableFormat(TrasladoDet.class,propertyNames, columnLabels,edits);
		final EventTableModel tm=new EventTableModel(getController().getPartidasSource(),tf);
		grid=ComponentUtils.getStandardTable();
		grid.setModel(tm);
		grid.setColumnControlVisible(false);
		//selectionModel=new EventSelectionModel<TrasladoDet>(getController().getPartidasSource());
	...
	public static void main(String[] args) {
		/*SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				Traslado sol=(Traslado)Services
						.getInstance().getHibernateTemplate()
						.get(Traslado.class
								,"8a8a81e9-3268382c-0132-689e06e2-0017");
				TrasladoAutomaticoController controller=new TrasladoAutomaticoController(sol);
