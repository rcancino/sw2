...
public class PedidoDetDolaresForm extends AbstractForm{
	
	
	
	private List<ProductoRow> productos;
	
	public PedidoDetDolaresForm(PedidoDetDolaresFormModel model) {
		super(model);		
	...
	private PedidoDetDolaresFormModel getDetModel(){
		return (PedidoDetDolaresFormModel)getModel();
	}
	...
	protected JComponent buildFormMainPanel() {
		...
		builder.append("Importe",addReadOnly("importeBruto"));
		builder.nextLine();
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				PedidoDetDolaresFormModel model=new PedidoDetDolaresFormModel(det);
				model.setSucursal(Services.getInstance().getConfiguracion().getSucursal());
				PedidoDetDolaresForm form=new PedidoDetDolaresForm(model);
