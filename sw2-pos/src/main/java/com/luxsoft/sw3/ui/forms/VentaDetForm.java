...
public class VentaDetForm extends AbstractForm{
	
	
	
	private List<ProductoRow> productos;
	
	public VentaDetForm(VentaDetFormModel model) {
		super(model);		
	...
	private VentaDetFormModel getDetModel(){
		return (VentaDetFormModel)getModel();
	}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				final VentaDet det=null;//VentaDet.getPedidoDet();
				
				VentaDetFormModel model=new VentaDetFormModel(det);
				model.setSucursal(Services.getInstance().getConfiguracion().getSucursal());
				VentaDetForm form=new VentaDetForm(model);
