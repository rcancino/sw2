...
public class SolicitudDeDepositoForm extends AbstractForm implements ActionListener{
	...
	public static SolicitudDeDeposito generar(OrigenDeOperacion origen) {
		...
		sol.setSucursal(Services.getInstance().getConfiguracion().getSucursal());
		sol.setFecha(Services.getInstance().obtenerFechaDelSistema());
	...
	public static void main(String[] args) {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				" where s.id=?";
				List<SolicitudDeDeposito> res=Services.getInstance().getHibernateTemplate().find(hql,"8a8a81e7-298e6e55-0129-8e8a3de5-0041");
				//consultar(res.get(0));
