 ...
 *
 */
public class ConteoDeInventarioVentasPanel extends AbstractMasterDatailFilteredBrowserPanel<Conteo, ConteoDet>{
	
	
	public ConteoDeInventarioVentasPanel() {
		super(Conteo.class);
	}
	...
	protected Conteo doEdit(Conteo bean) {
		...
		if(validarInventarioFijado(bean.getFecha())){
			...
			final ConteoForm2 form=new ConteoForm2(controller);
			form.open();
			if(!form.hasBeenCanceled()){
		...
		if(contados.intValue()==0){			
			...
			if((user!=null) && user.hasRole(POSRoles.CONTROLADOR_DE_INVENTARIOS.name())){
				...
				try {
					...
					boolean recalcular=MessageUtils.showConfirmationMessage("¿ Recalcular Productos ?", "Conteo de Inventario");
					if(recalcular){
						calcularExistencias();
					}else{
