 ...
 *
 */
public class PedidoFormView extends AbstractForm {

	private final Font TITLE_LABEL_FONT=new java.awt.Font("SansSerif", 1, 13);
	
	PedidoHeader header;	

	public PedidoFormView(DefaultFormModel model) {
		super(model);
	...
	public static void showPedido(final Pedido pedido){
		...
		PedidoFormView form=new PedidoFormView(model);
		form.open();
	}
	...
	public static void main(String[] args) {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				Services.getInstance().getHibernateTemplate().setMaxResults(1);
				Pedido pedido=(Pedido)Services.getInstance().getHibernateTemplate()
					.find("from Pedido p ").get(0);
