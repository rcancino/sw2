...
import com.luxsoft.sw3.ui.forms.CompraController;
import com.luxsoft.sw3.ui.forms.CompraForm;

public class AlcancesPanel_Pos extends FilteredBrowserPanel<Alcance>{
	
	

	public AlcancesPanel_Pos() {
		super(Alcance.class);
		...
		if(!getSelected().isEmpty()){
			if(sucursal!=null){
				...
				if(selected!=null){
					...
					Proveedor prov=Services.getInstance().getProveedorManager().buscarPorNombre(provNombre);
					if(prov!=null){
						...
						compra.setSucursal(Services.getInstance().getConfiguracion().getSucursal());
						for(Alcance a:result){
							CompraUnitaria cu=new CompraUnitaria();
							Producto prod=Services.getInstance().getProductosManager().buscarPorClave(a.getCLAVE());
							...
							cu.setSucursal(compra.getSucursal());
							compra.agregarPartida(cu);
							Services.getInstance().getComprasManager().asignarPrecioDescuento(cu);
							
						}
						CompraController controller=new CompraController(compra);
						...
						if(!form.hasBeenCanceled()){
							...
							compra=Services.getInstance().getComprasManager().save(compra);
	...
	public static void showInDialog(){
		SXAbstractDialog dialog=new SXAbstractDialog(""){
			...
			protected JComponent buildContent() {
				...
				AlcancesPanel_Pos alcancePanel=new AlcancesPanel_Pos();
	...
	public static class AlcanceForm extends SXAbstractDialog{
		...
		private void init(){
			...
			todasLasSucursales.setEnabled(false);
