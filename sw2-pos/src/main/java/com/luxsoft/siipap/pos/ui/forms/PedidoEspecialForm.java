 ...
 *
 */
public class PedidoEspecialForm extends AbstractForm implements ActionListener,ListSelectionListener{
	...
	public PedidoEspecialForm(final PedidoEspecialFormController controller) {
		super(controller);
	...
	public PedidoEspecialFormController getController(){
		return (PedidoEspecialFormController)getModel();
	}
	...
	protected JComponent buildFormPanel() {
		...
		builder.append("Socio",getControl("socio"),5);
		builder.nextLine();
	...
	protected JPanel buildToolbarPanel(){
		...
		JButton medidaEspecialBtn=new JButton("M. Especial");
		medidaEspecialBtn.addActionListener(EventHandler.create(ActionListener.class, this, "insertarPartidaEspecial"));
		
		
		Action maquila=new com.luxsoft.siipap.swing.actions.DispatchingAction(this,"asignarEntradaPorMaquila");		
		maquila.putValue(Action.NAME, "Asignar MAQ");
		maquila.putValue(Action.MNEMONIC_KEY, KeyEvent.VK_M);
		
		Action com=new com.luxsoft.siipap.swing.actions.DispatchingAction(this,"asignarEntradaPorCompra");		
		com.putValue(Action.NAME, "Asignar COM");
	...
	public void insertarPartidaEspecial(){
		getController().insertarPartidaEspecial();
		((JXTable)grid).packAll();
		grid.requestFocusInWindow();
	}
	...
	public void asignarEntradaPorCompra(){
		if(getController().getPedido().isFacturado())
			return;
		if(selectionModel.isSelectionEmpty())
			return;
		PedidoDet selected=(PedidoDet)selectionModel.getSelected().get(0);
		getController().asignarEntradaPorCompra(selected);
		grid.packAll();
	}
	
	public void asignarEntradaPorMaquila(){
		if(getController().getPedido().isFacturado())
			return;
		if(selectionModel.isSelectionEmpty())
			return;
		PedidoDet selected=(PedidoDet)selectionModel.getSelected().get(0);
		getController().asignarEntradaPorMaquila(selected);
		grid.packAll();
	}
	...
	public static void main(String[] args) {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				PedidoEspecialFormController model=new PedidoEspecialFormController();
				PedidoEspecialForm form=new PedidoEspecialForm(model);
				form.open();
