 ...
 *
 */
public  class SelectorDeFacturasParaCOD 
							extends AbstractSelector<VentaContraEntrega>{
	
	protected Periodo periodo=Periodo.getPeriodo(-10);
	Sucursal suc;
	
	
	public SelectorDeFacturasParaCOD() {
	...
	protected List<VentaContraEntrega> getData() {
		String sql="select a.docto as documento,b.folio as pedido" +
		",b.folio,a.fecha,a.creado as facturado,a.nombre,a.origen,a.total" +
		",a.fpago,a.ce as contraEntrega,a.cargo_id as id"+ 
		  " from sx_ventas a  "+
		  " join sx_pedidos b on(a.pedido_id=b.pedido_id) "+
		  " where b.instruccion_id is null "+
	...
	public static Venta seleccionar(){
		SelectorDeFacturasParaCOD selector=new SelectorDeFacturasParaCOD();
		selector.setSelectionMode(ListSelection.SINGLE_SELECTION);
		selector.open();
		if(!selector.hasBeenCanceled()){
			VentaContraEntrega v=selector.getSelected();
			return Services.getInstance().getFacturasManager().getVentaDao().buscarVentaInicializada(v.getId());
		}		
		return null;
	}
