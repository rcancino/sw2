 ...
 *
 */
public class CorteDeCajaDepositoFormModel extends DefaultFormModel {
		

	public CorteDeCajaDepositoFormModel() {
	...
	protected void init() {
		...
		caja.setTipo(Caja.Tipo.DEPOSITO);
		caja.setOrigen(null);
	...
	public void actualizarPagos(){
		...
		String hql="select sum(p.total) from PagoConDeposito p " +
				"where p.primeraAplicacion=? " +
				"  and p.sucursal.id=?" +				
				"  and p.origenAplicacion=\'@ORIGEN\' " +
				"  and p.carTipoAplicacion!=\'TES\'" +
		...
		String hql2="select sum(p.total) from PagoConDeposito p " +
			"where p.fecha=? " +
			"  and p.sucursal.id=?" +				
			"  and p.origenAplicacion=\'@ORIGEN\' " +
			"  and p.carTipoAplicacion!=\'TES\'" +
