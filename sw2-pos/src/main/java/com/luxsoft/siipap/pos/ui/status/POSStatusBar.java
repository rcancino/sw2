...
import com.luxsoft.siipap.swing.controls.TrinagleSquareWindowsCornerIcon;

public class POSStatusBar extends StatusBar{
	...
	private JComponent getComponents(){
		...
		final POSDataBaseLocator dbLocator=new POSDataBaseLocator();
		final POSMemoryMonitor monitor=new POSMemoryMonitor();
		final UserLocator userLocator=new UserLocator();
		
		builder.add(dbLocator.getControl(),cc.xy(1, 1));
		builder.add(monitor.getControl(),cc.xy(3, 1));
		builder.add(userLocator.getControl(),cc.xy(5, 1));
