 ...
 *
 */
public class ReporteDeAlcancesForm extends SWXAction{
	...
	protected void execute() {
		...
		if(!form.hasBeenCanceled()){
			...
			ReportUtils2.runReport("compras/MesesDeInventario.jasper", form.getParametros());
			}
			else {
				POSDBUtils.whereWeAre();
				System.out.println("Parametros enviados: "+form.getParametros());
				ReportUtils2.runReport("compras/GeneralDeAlcance.jasper", form.getParametros());	
			}
	...
	public static void run(){
		ReporteDeAlcancesForm action=new ReporteDeAlcancesForm();
		action.execute();
	}
	...
	public  class ReportForm extends SXAbstractDialog{
		...
		private JComboBox buildLineaControl() {
			...
			final EventList source = GlazedLists.eventList(Services.getInstance().getHibernateTemplate().find("from Linea l order by l.nombre"));
