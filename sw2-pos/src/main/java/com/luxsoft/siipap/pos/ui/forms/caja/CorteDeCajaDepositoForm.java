 ...
 *
 */
public class CorteDeCajaDepositoForm extends AbstractForm {

	public CorteDeCajaDepositoForm(CorteDeCajaDepositoFormModel model) {
		super(model);
	...
	public static Caja registrarCorte(){
		final CorteDeCajaDepositoFormModel model=new CorteDeCajaDepositoFormModel();
		
		final CorteDeCajaDepositoForm form=new CorteDeCajaDepositoForm(model);
		form.open();
		if(!form.hasBeenCanceled()){
			Caja bean=model.commit();
			bean=Services.getInstance().getCorteDeCajaManager().registrarCorteDeCajaDeposito(bean);			
			return bean;
		}
		return null;
	}
