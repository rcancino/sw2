...
/**
 * Implementacion de BeanValidator para hibernate utilizando etiquetas cortas
 * 
 * @author Ruben Cancino
 *
 */
public class ShortBeanValidationImpl implements BeanValidator {
	...
	*/
	...
	public ValidationResult validate(final Object bean){
		final Class clazz=bean.getClass();
		//final String role=ClassUtils.getShortName(clazz);
		final PropertyValidationSupport support =new PropertyValidationSupport(clazz,"");
		...
		for(InvalidValue iv:invalid){
			...
			//String propName=iv.getPropertyName();						
			support.addError("", iv.getMessage());
		}
		final ValidationResult res=support.getResult();
		return res;
	}
