 ...
 *
 */
public class PedidoForm extends AbstractForm implements ActionListener,ListSelectionListener{
	...
	public PedidoForm(final PedidoController controller) {
		super(controller);
		...
		/*if(getController().getPedido().getId()==null){
			Cliente papel=ServiceLocator2.getClienteManager().get(1L);
			getController().getPedido().setCliente(papel);
		}*/
	...
	protected JComponent buildGridPanel(){
		String[] propertyNames={"clave","descripcion","producto.gramos"
				,"producto.calibre","cantidad","descripcionCorte"
				//,"backOrder"
