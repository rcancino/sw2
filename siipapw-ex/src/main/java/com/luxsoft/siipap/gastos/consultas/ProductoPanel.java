...
public class ProductoPanel extends AbstractGastosGroupByPanel {
	

	public ProductoPanel(final EventList source){
		super(source);
	}
	...
	protected Comparator getGroupComparator(){
		return GlazedLists.beanPropertyComparator(AnalisisDeGasto.class, "descripcion");
	}
	...
	private class GroupByFunction implements Function<List<AnalisisDeGasto>, GroupByBean>{
		public GroupByBean evaluate(List<AnalisisDeGasto> sourceValue) {
			...
			bean.setKey(sourceValue.get(0).getDescripcion());
			double val=0;
