...
public class ProveedorlPanel extends AbstractGastosGroupByPanel {
	

	public ProveedorlPanel(final EventList source){
		super(source);
	}
	...
	protected Comparator getGroupComparator(){
		return GlazedLists.beanPropertyComparator(AnalisisDeGasto.class, "proveedor");
	}
	...
	private class GroupByFunction implements Function<List<AnalisisDeGasto>, GroupByBean>{
		public GroupByBean evaluate(List<AnalisisDeGasto> sourceValue) {
			...
			bean.setKey(sourceValue.get(0).getProveedor());
			double val=0;
