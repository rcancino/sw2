...
public class RubrosPanel extends AbstractGastosGroupByPanel {
	

	public RubrosPanel(final EventList source){
		super(source);
	}
	...
	protected Comparator getGroupComparator(){
		return GlazedLists.beanPropertyComparator(AnalisisDeGasto.class, "rubro");
	}
	...
	private class GroupByFunction implements Function<List<AnalisisDeGasto>, GroupByBean>{
		public GroupByBean evaluate(List<AnalisisDeGasto> sourceValue) {
			...
			bean.setKey(sourceValue.get(0).getRubro());
			double val=0;
