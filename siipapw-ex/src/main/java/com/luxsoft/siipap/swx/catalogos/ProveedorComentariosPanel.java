 ...
 *
 */
public class ProveedorComentariosPanel extends JPanel{
	...
	public ProveedorComentariosPanel(final PresentationModel model){
		this.model=model;
	...
	private void updateGrid(){
		partidas.clear();
		partidas.addAll(getProveedor().getComentarios());
	}
	...
	public void commit(){
		...
		boolean ok=getProveedor().agregarComentario(c);
		if(ok)
			updateGrid();
		else
			nuevo();
	...
	public void eliminar(){
		...
		for(int i=0;i<selection.size();i++){
			...
			boolean res=getProveedor().removeComentario(c);
			if(!update)
				update=res;
		}
		if(update)
