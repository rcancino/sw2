...
import com.luxsoft.sw3.cxp.consultas.FacturaDeComprasController;
 ...
 * 
 * @TODO Migrar a la estructura de AbstractForm2 contemplar: 
 * 
 *   Que la edicion/modificacion/eliminacion de partidas  funcione correctamente actualizando totales,y aplicando las reglas adecuadas:
 *     ej: Si es factura analizable , si es anticipo etc.
 *     
 *     Contmplar que cuando se trate de anticipo solo permita ver las facturas de anticipo (CXPFActura.anticipo !=null)
 *     
 *     Que permita ver las facturas de Analisi de Flete,Gastos TRS
 *     
 *   
 *
 */
public class RequisicionDeComprasForm extends AbstractMasterDetailForm{
	
	protected Logger logger=Logger.getLogger(getClass());

	public RequisicionDeComprasForm(RequisicionDeComprasFormModel model) {
		super(model);
	...
	public RequisicionDeComprasFormModel getMainModel(){
		return (RequisicionDeComprasFormModel)getModel();
	}
	...
	protected JComponent createCustomComponent(String property) {
		if("proveedor".equals(property)){
			...
			JComponent c=BasicComponentFactory.createLabel(getMainModel().totalModel, FormatUtils.getToStringFormat());
			return c;
		}
		return null;
	}
	...
	public void insertarPorContrarecibo(){
		...
		getMainModel().procesarFacturas(numeros);
	}
	...
	public static Requisicion showForm(Requisicion bean,boolean readOnly){
		RequisicionDeComprasFormModel model=new RequisicionDeComprasFormModel(bean);
		model.setReadOnly(readOnly);
		final RequisicionDeComprasForm form=new RequisicionDeComprasForm(model);
		form.open();
		...
		SwingUtilities.invokeAndWait(new Runnable(){
			public void run() {
				...
				if(bean!=null){
					...
					RequisicionDeComprasForm.showObject(bean);
