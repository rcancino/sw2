...
/**
 * Nuevo panel para la administración de Requisiciones de compras
 * 
 * @author Ruben Cancino
 *
 */
public class RequisicionDeComprasPanel extends  AbstractMasterDatailFilteredBrowserPanel<Requisicion,RequisicionDe>{

	
	public RequisicionDeComprasPanel() {
	...
	protected void agregarMasterProperties() {
		...
		addProperty("id","afavor","fecha","concepto.clave","moneda","tipoDeCambio","total.amount"
				,"estado.name"
				,"pagoCxp.id"
				,"origen"
				,"formaDePago"
		...
		addLabels("Id","Proveedor","Fecha","Concepto","Moneda","TC","Total","estado","Pago (Apl)","Origen"				
				,"F.P"
				,"Banco"
				,"Referencia"
				,"Comentario"
	...
	protected Requisicion doInsert() {
		return RequisicionDeComprasController.generarRequisicion(periodo);
	}
	
	@Override
	public boolean doDelete(Requisicion bean) {
		if(bean.getPago()!=null){
			MessageUtils.showMessage("Requisición con pago en tesorería no se puede eliminar", "Requisiciones de compras");
			return false;
		}
		RequisicionDeComprasController.eliminarRequisicion(bean);
		return true;
	...
	protected Requisicion doEdit(Requisicion bean) {
		return RequisicionDeComprasController.editar(bean, periodo);
	}
	...
	private class RequisicionPagable implements Predicate{
		...
		//	if(selected==null ) return false;
			if(selected==null  ) 
				return false;
			if(selected.getPago() == null)
			    return false ;
			if(selected.getPagoCxp()!=null)
				return false ;
			 return true;
