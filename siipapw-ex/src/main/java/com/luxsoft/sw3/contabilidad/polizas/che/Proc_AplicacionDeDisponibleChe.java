...
public class Proc_AplicacionDeDisponibleChe implements IProcesador{
	...
	public void procesar(Poliza poliza, ModelMap model) {
		...
		for(final Pago pago:pagos){
			...
			if(!DateUtils.isSameDay(poliza.getFecha(), pago.getPrimeraAplicacion())){
				...
				if(!pAplicacion.getDetalle().getOrigen().equals("CHE")){
				...
				PolizaDetFactory.generarPolizaDet(poliza, "203","DIVR01", true, totalAplicado, desc2, ref1, ref2, asiento);
				...
				PolizaDetFactory.generarPolizaDet(poliza,"113", pago.getClave(), false, totalAplicado, "Clientes CHE aplic SAF", ref1, ref2, asiento);
