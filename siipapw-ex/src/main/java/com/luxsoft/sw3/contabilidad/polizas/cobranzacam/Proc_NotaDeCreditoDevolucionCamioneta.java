...
public class Proc_NotaDeCreditoDevolucionCamioneta implements IProcesador{
	...
	void procesar(Poliza poliza, Abono entidad) {
		...
		if(nota.getOrigen().equals(OrigenDeOperacion.CAM)){
			...
			PolizaDetFactory.generarPolizaDet(poliza, "405", "DVTA02", true, importe,desc2, ref1, ref2, asiento);
			...
			PolizaDetFactory.generarPolizaDet(poliza,"105", dev.getVenta().getSucursal().getId().toString(), false, total,desc2, ref1, ref2, asiento);
