...
public class Proc_CobranzaFichasChe implements IProcesador{
			...
			for(Pago p:lpagos){
				if( (p instanceof PagoConCheque) || (p instanceof PagoConEfectivo)){
					if(DateUtils.isSameDay(p.getPrimeraAplicacion(), poliza.getFecha())){
						totalAplicado=p.getAplicado(poliza.getFecha());
						...
						PolizaDetFactory.generarPolizaDet(poliza,"113", p.getClave(), false, totalAplicado, "Documentos Por Cobrar CHE", ref1, ref2, asiento);
						//Cargo Iva en ventas por trasladar
