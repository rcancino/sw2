 ...
 *
 */
public class PolizaDeCobroDeChequeEnTransitoController extends AbstractPolizaMultipleManager{
	
	
	public Poliza existente(final Poliza poliza){
		String hql="from Poliza p where p.clase=? and p.referencia=?";
		Object values[]={"COBRO CHEQUE TRANSITO",poliza.getReferencia()};
	...
	public Poliza actualizar(Poliza poliza){
		...
		List<CargoAbono> cargos=getHibernateTemplate()
				.find("from CargoAbono c left join fetch c.pago p where c.id=?",id);
	...
	public static void main(String[] args) {
		DBUtils.whereWeAre();
		PolizaDeCobroDeChequeEnTransitoController controller=new PolizaDeCobroDeChequeEnTransitoController();
