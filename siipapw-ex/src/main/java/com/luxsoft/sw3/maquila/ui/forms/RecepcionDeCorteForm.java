...
/**
 * Forma para la generación y mantenimiento de recepciones de corte
 * 
 * @author Ruben Cancino Ramos
 *
 */
public class RecepcionDeCorteForm extends AbstractForm implements ListSelectionListener{
	

	public RecepcionDeCorteForm(final RecepcionDeCorteFormModel model) {
		super(model);
		setTitle("Recepción de corte     ");
		if(model.getModel("almacen")!=null){
	...
	public RecepcionDeCorteFormModel getController(){
		return (RecepcionDeCorteFormModel)getModel();
	}
	...
	private JComponent buildAlmacenControl(final ValueModel vm) {
		if (model.getValue("id") == null) {
			...
			final TextFilterator filterator = GlazedLists.textFilterator(new String[] { "maquilador.nombre" });
	...
	private EventSelectionModel<RecepcionDeCorteDet> selectionModel;
	...
	protected JComponent buildGridPanel(){
		...
		final TableFormat tf=GlazedLists.tableFormat(RecepcionDeCorteDet.class,propertyNames, columnLabels);
		final EventTableModel tm=new EventTableModel(getController().getPartidasSource(),tf);
		grid=ComponentUtils.getStandardTable();
		grid.setModel(tm);
		selectionModel=new EventSelectionModel<RecepcionDeCorteDet>(getController().getPartidasSource());
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				RecepcionDeCorteFormModel controller=new RecepcionDeCorteFormModel();
				RecepcionDeCorteForm form=new RecepcionDeCorteForm(controller);
				form.open();
