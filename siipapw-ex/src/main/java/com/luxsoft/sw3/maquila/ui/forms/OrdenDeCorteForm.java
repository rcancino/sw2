...
/**
 * Fomra para la generación y mantenimiento de ordenes de corte
 * 
 * @author Ruben Cancino Ramos
 *
 */
public class OrdenDeCorteForm extends AbstractForm implements ListSelectionListener{
	
	

	public OrdenDeCorteForm(final OrdenDeCorteFormModel model) {
		super(model);
		setTitle("Orden de Corte     ");
	...
	public OrdenDeCorteFormModel getController(){
		return (OrdenDeCorteFormModel)getModel();
	}
	...
	private EventSelectionModel<OrdenDeCorteDet> selectionModel;
	...
	protected JComponent buildGridPanel(){
		...
		final TableFormat tf=GlazedLists.tableFormat(OrdenDeCorteDet.class,propertyNames, columnLabels);
		final EventTableModel tm=new EventTableModel(getController().getPartidasSource(),tf);
		grid=ComponentUtils.getStandardTable();
		grid.setModel(tm);
		selectionModel=new EventSelectionModel<OrdenDeCorteDet>(getController().getPartidasSource());
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				OrdenDeCorteFormModel controller=new OrdenDeCorteFormModel();
				OrdenDeCorteForm form=new OrdenDeCorteForm(controller);
				form.open();
