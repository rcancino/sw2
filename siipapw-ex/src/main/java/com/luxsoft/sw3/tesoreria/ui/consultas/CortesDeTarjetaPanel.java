 ...
 *
 */
public class CortesDeTarjetaPanel extends AbstractMasterDatailFilteredBrowserPanel<CorteDeTarjeta,CorteDeTarjetaDet> {
		...
		if(selected!=null){
			if(selected.getTipoDeTarjeta().startsWith("AMEX")){
				...
				if(MessageUtils.showConfirmationMessage(msg, "Cambio de fecha")){
					...
					for(CargoAbonoPorCorte ca:target.getAplicaciones()){
						ca.getCargoAbono().setFechaCobro(fecha);
