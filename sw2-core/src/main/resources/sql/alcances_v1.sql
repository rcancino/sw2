...
FROM (
SELECT X.PRODUCTO_ID,X.CLAVE,SUM(X.CANTIDAD/X.FACTORU) AS EXI,0 AS VTA,0 AS DEV,0 AS PEND FROM SX_EXISTENCIAS X WHERE X.YEAR=YEAR(@FECHA_FIN) AND  X.MES=MONTH(@FECHA_FIN) AND  X.SUCURSAL_ID LIKE @SUCURSAL_ID GROUP BY X.CLAVE,X.PRODUCTO_ID
UNION
