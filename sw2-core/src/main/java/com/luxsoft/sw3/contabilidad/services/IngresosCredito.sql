...
JOIN sw_sucursales S ON(S.SUCURSAL_ID=B.SUCURSAL_ID) 
where b.fecha=? and b.ORIGEN NOT IN('VENTA_MOSTRADOR','VENTA_CAMIONETA') 
union
...
JOIN sw_sucursales S ON(S.SUCURSAL_ID=B.SUCURSAL_ID) 
where A.fecha=? and A.ORIGEN NOT IN('MOS','CAM')  and b.conciliado=false AND A.ABONO_ID NOT IN(SELECT X.ABONO_ID FROM SX_CXC_APLICACIONES X JOIN SX_VENTAS V ON(V.CARGO_ID=X.CARGO_ID) WHERE X.ABONO_ID=A.ABONO_ID AND V.TIPO='TES') 
