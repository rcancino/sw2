...
public class ComprobantesDigitalesManagerImpl extends CFDSupportImpl implements ComprobantesDigitalesManager{
	...
	public ComprobanteFiscal generarComprobante(Venta venta) {
		...
		// Version 2.2
		cfd.setMoneda(venta.getMoneda().getCurrencyCode());
		cfd.setTipoCambio(BigDecimal.valueOf(venta.getTc()).toString());
		cfd.setTipoDeComprobante(TipoDeComprobante.INGRESO);
		cfd.setFormaDePago("PAGO EN UNA SOLA EXHIBICION");

		String cuenta=venta.getComentarioCancelacionDBF();
		
		if (cuenta.equals("")){
			cfd.setMetodoDePago("NO IDENTIFICADO");
		}else{			
			if(venta.getFormaDePago().equals(FormaDePago.DEPOSITO)){
				cfd.setMetodoDePago("CHEQUE");
				cfd.setNumCtaPago(venta.getComentarioCancelacionDBF());
			}else{
				cfd.setMetodoDePago(venta.getFormaDePago().name());
				cfd.setNumCtaPago(venta.getComentarioCancelacionDBF());
			}
		}
		registrarLugarDeExpedicion(cfd, venta.getSucursal());
	...
	public ComprobanteFiscal generarComprobante(NotaDeCredito nota) {
		...
		cfd.setTipoDeComprobante(TipoDeComprobante.INGRESO);
		cfd.setFormaDePago("PAGO EN UNA SOLA EXHIBICION");
		cfd.setMetodoDePago("EFECTIVO");
		cfd.setNumCtaPago("1234");
		registrarLugarDeExpedicion(cfd, nota.getSucursal());
		...
		cfd.setMoneda(nota.getMoneda().getCurrencyCode());
	...
	public ComprobanteFiscal generarComprobante(final NotaDeCargo notaDeCargo){
		...
		cfd.setMetodoDePago("CHEQUE");
		cfd.setNumCtaPago("1234");		
		registrarLugarDeExpedicion(cfd, notaDeCargo.getSucursal());
