 ...
 *
 */
public class ImportadorCentralManager implements InitializingBean{
	...
	protected void doImportar(final EntityLog log){
		if(log.getTipo().equals(EntityLog.Tipo.BAJA)){
			ServiceLocator2.getHibernateTemplate().delete(log.getBean());			
			logger.info("Entidad exitosamente ELIMINADA : "+log);
		}else{
